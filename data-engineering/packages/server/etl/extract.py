...
Each partner sends a different shape: partner_a is a semicolon-delimited CSV,
partner_b is JSON with a nested contact object. Parsing only pulls fields out
of whatever shape the file has — it does not validate or normalize them. That
happens in `transform.py`, once a row has been staged, so a parse failure and
a validation failure are never confused with each other.
"""
